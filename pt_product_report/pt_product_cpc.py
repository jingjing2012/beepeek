...
df_asin_cpc = sql_engine.connect_pt_product(config.sellersprite_hostname, config.sellersprite_password,
                                            config.sellersprite_database, sql.sql_asin_cpc)
...
if df_asin_cpc.empty and df_get_group.empty:
    ...
    while id_start < id_end:
        # 1.数据连接
        sql_asin = "select * from " + path.pt_product_get_cpc + " where status=1 and id between " + str(
            id_start) + " and " + str(id_start + id_increment)
