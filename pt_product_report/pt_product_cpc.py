...
for site in sites:
    ...
    df_asin_cpc = sql_engine.connect_pt_product(config.sellersprite_hostname, config.sellersprite_password,
                                                sellersprite_database, sql.sql_asin_cpc)
    df_get_group = sql_engine.connect_pt_product(config.sellersprite_hostname, config.sellersprite_password,
                                                 sellersprite_database, sql.sql_get_group)

    if df_asin_cpc.empty and df_get_group.empty:
        ...
        while id_start < id_end:
            ...
            df_opportunity['site'] = site

            # tag表
            df_cpc_tag = df_opportunity[['asin', 'site', '加权CPC', '市场蓝海度', '数据更新时间']]

            mr_list = ['加权CPC', '市场蓝海度']
            for mr in mr_list:
                calculation_util.get_mround(df_cpc_tag, mr, mr + '分布', 0.05)

            df_cpc_tag['data_id'] = df_cpc_tag['asin'] + " | " + df_cpc_tag['site'] + " | " + update_date

            # 字段整合
            df_kw_cpc = df_kw_cpc[
                ['asin', 'site', 'keyword', 'bid_rangeMedian', 'bid_rangeEnd', 'ASIN_KW相关度', 'rank', '数据更新时间']]
            ...
            df_opportunity = df_opportunity[
                ['asin', 'site', 'price', 'recommend', 'keyword', '加权CPC', '预期CR', '转化净值', '预期CPC', 'CPC因子', '市场蓝海度',
                 '数据更新时间']]
            ...
            df_group = df_group[['ASIN', 'site', '价格', '推荐度', '市场蓝海度']]
            ...
            sql_engine.data_to_sql(
                df_group, path.pt_product_get_group, 'append', db_util.connet_sellersprite_db_sql(sellersprite_database)
            )
        ...
        sql_engine.connect_product(
            config.sellersprite_hostname, config.sellersprite_password, sellersprite_database,
