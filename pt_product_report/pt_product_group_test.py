...
def connect_pt_product(hostname, database, product_sql_pt):
    conn_pt = mysql.connector.connect(host=hostname, user=config.oe_username, password=config.oe_password,
                                      database=database)
    cur = conn_pt.cursor()
...
# 数据增删改
def connect_product(hostname, database, product_sql):
    conn_oe = mysql.connector.connect(host=hostname, user=config.oe_username, password=config.oe_password,
                                      database=database)
    cur = conn_oe.cursor()
...
# 创建索引
# connect_product(config.sellersprite_hostname, sellersprite_database, sql.create_index_sql_relevance_1)
# connect_product(config.sellersprite_hostname, sellersprite_database, sql.create_index_sql_relevance_2)
#
# connect_product(config.sellersprite_hostname, sellersprite_database, sql.create_index_sql_supplement)
# connect_product(config.sellersprite_hostname, sellersprite_database, sql.duplicate_sql_supplement)
...
sellersprite_database = "sellersprite_202407"

# 更新日期
update_date = str(sellersprite_database)[-6:-2] + "-" + str(sellersprite_database)[-2:] + "-01"

start_time = time.time()
df_famous_brand = connect_pt_product(config.oe_hostname, path.product_database, sql.famous_brand_sql)
df_holiday = connect_pt_product(config.oe_hostname, path.product_database, sql.holiday_sql)

# 机器学习训练数据准备
df_group_knn = connect_pt_product(config.oe_hostname, config.product_database, sql.sampling_knn_sql)
...
for page in range(total_pages):
    ...
    df_product = connect_pt_product(config.sellersprite_hostname, sellersprite_database, sql_asin)
    if df_product.empty:
        break

    df_relation = connect_pt_product(config.sellersprite_hostname, sellersprite_database, sql_traffic)
    df_relation_add = connect_pt_product(config.sellersprite_hostname, sellersprite_database, sql_traffic_add)
    ...
    for con_i in product_con_list_1:
        convert_type(df_related_traffic, con_i, 2)
    ...
    # 聚合表
    df_group = df_group[
        ['related_asin', 'price', 'recommend', 'blue_ocean_estimate', '相关竞品款数', '有销额竞品款数', '有销额推荐达标款数', '有销额竞品款数占比',
...
# 类目利基去重
connect_product(config.oe_hostname, path.product_database, sql.pmi_rank_sql)
connect_product(config.oe_hostname, path.product_database, sql.duplicate_sql1)
connect_product(config.oe_hostname, path.product_database, sql.duplicate_sql2)
# 父体去重
connect_product(config.oe_hostname, path.product_database, sql.duplicate_sql3)
# 历史开售产品去重
connect_product(config.oe_hostname, path.product_database, sql.duplicate_sql4)
connect_product(config.oe_hostname, path.product_database, sql.duplicate_sql5)
connect_product(config.oe_hostname, path.product_database, sql.duplicate_sql6)

connect_product(config.oe_hostname, path.product_database, sql.update_sql_product_group_tag)
