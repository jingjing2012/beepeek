...
df_famous_brand = sql_engine.connect_pt_product(
    config.oe_hostname, config.oe_password, path.product_database, sql.famous_brand_sql
)

df_holiday = sql_engine.connect_pt_product(
    config.oe_hostname, config.oe_password, path.product_database, sql.holiday_sql
)
...
for site in sites:
    ...
    df_relevance_asins = sql_engine.connect_pt_product(
        config.sellersprite_hostname, config.sellersprite_password, sellersprite_database, sql.sql_relevance_asins
    )

    df_group = sql_engine.connect_pt_product(
        config.oe_hostname, config.oe_password, config.product_database, sql.sampling_group_sql
    )
    ...
    if df_group.empty and (not df_relevance_asins.empty):
        ...
        while id_start < id_end:
            ...
            df_product = sql_engine.connect_pt_product(
            ...
            df_duplicate = sql_engine.connect_pt_product(
                config.sellersprite_hostname,
                config.sellersprite_password,
                sellersprite_database,
                db_util.group_duplicate_sql(id_start, id_increment, path.pt_product_duplicate)
            )

            df_relation = sql_engine.connect_pt_product(
                config.sellersprite_hostname,
                config.sellersprite_password,
                sellersprite_database,
                db_util.group_traffic_sql(id_start, id_increment, path.pt_relevance_asins, path.pt_relation_traffic)
            )

            df_relation_old = sql_engine.connect_pt_product(
                config.sellersprite_hostname,
                config.sellersprite_password,
                sellersprite_database,
                db_util.group_traffic_sql(id_start, id_increment, path.pt_relevance_asins_old,
                                          path.pt_relation_traffic_old)
            )

            df_relation_older = sql_engine.connect_pt_product(
                config.sellersprite_hostname,
                config.sellersprite_password,
                sellersprite_database,
                db_util.group_traffic_sql(id_start, id_increment, path.pt_relevance_asins_older,
            ...
            # 3.1开售月数
            calculate_util.month_available(df_relation)
            ...
            df_traffic['知名品牌'] = np.where(df_traffic['brand'] == 'Generic', 1,
                                          df_traffic['知名品牌_1'] + df_traffic['知名品牌_2'])
            ...
            df_traffic['数据更新时间'] = data_cleaning_util.convert_date(df_traffic, '数据更新时间')
            ...
            # 有销额竞品款数
            traffic_revenue_count = calculation_util.product_count(traffic_revenue_pass, 'related_asin', 'asin',
                                                                   '有销额竞品款数')
            ...
            traffic_recommend_count = calculation_util.product_count(traffic_recommend, 'related_asin', 'asin',
                                                                     '有销额推荐达标款数')
            ...
            traffic_df['TOP5月均销额_score'] = common_util.get_cut(traffic_df, 'TOP5月均销额', para.s_sales_bins,
                                                               para.s_sales_labels)
            traffic_df['TOP5月均销额_tag'] = common_util.get_cut(traffic_df, 'TOP5月均销额', para.s_sales_bins,
                                                             para.s_sales_tags)
            ...
            # 价格集中度
            traffic_df['价格集中度'] = np.where(traffic_df['价格中位数'] > 0, 1 - traffic_df['价格标准差'] / traffic_df['价格中位数'],
                                           np.nan)
            ...
            # 预估平均毛利率
            traffic_gross_margin = calculation_util.product_mean(traffic_revenue_pass, 'related_asin', '预估毛利率',
                                                                 '预估平均毛利率')

            traffic_df = traffic_df.merge(traffic_gross_margin, how='left', on='related_asin')

            traffic_df['预估平均毛利率_score'] = common_util.get_cut(traffic_df, '预估平均毛利率', para.m_gross_bins,
                                                              para.m_gross_labels)
            ...
            # 加权FBA运费
            traffic_fba_fees_mean = calculation_util.product_mean(traffic_fba_fees, 'related_asin', 'fba_fees',
                                                                  '加权FBA运费')
            ...
            # 直发FBM产品数
            traffic_fbm_cal_count = calculation_util.product_count(traffic_fbm_cal_df, 'related_asin', 'asin',
                                                                   '直发FBM产品数')

            traffic_df = traffic_df.merge(traffic_fbm_cal_count, how='left', on='related_asin')

            # 直发FBM产品占比
            traffic_df['直发FBM产品占比'] = np.where(traffic_df['有销额竞品款数'] > 0,
                                               traffic_df['直发FBM产品数'] / traffic_df['有销额竞品款数'],
                                               np.nan)

            # 直发FBM销额
            traffic_fbm_cal_revenue = calculation_util.product_sum(traffic_fbm_cal_df, 'related_asin',
                                                                   'monthly_revenue',
                                                                   '直发FBM销额')
            ...
            # AMZ直营销额
            traffic_amz_revenue = calculation_util.product_sum(traffic_amz_df, 'related_asin', 'monthly_revenue',
                                                               'AMZ直营销额')
            ...
            traffic_df['AMZ直营销额占比_score'] = common_util.get_cut(traffic_df, 'AMZ直营销额占比', para.i_amz_bins,
                                                                para.i_amz_labels)
            ...
            traffic_df['大牌商标销额占比_tag'] = common_util.get_cut(traffic_df, '大牌商标销额占比', para.i_famous_bins,
                                                             para.i_famous_tags)
            ...
            traffic_df['冒出品平均LQS_score'] = common_util.get_cut(traffic_df, '冒出品平均LQS', para.i_lqs_bins,
                                                               para.i_lqs_labels)
            ...
            traffic_df['冒出品视频占比_score'] = common_util.get_cut(traffic_df, '冒出品视频占比', para.i_video_bins,
                                                              para.i_video_labels)
            ...
            # 冒出品有QA产品数
            traffic_good_qa_count = calculation_util.product_count(traffic_good_qa_df, 'related_asin', 'asin',
                                                                   '冒出品有QA产品数')
            ...
            traffic_df['动销品平均星级_tag'] = common_util.get_cut(traffic_df, '动销品平均星级', para.i_rating_bins,
                                                            para.i_rating_tags)
            ...
            traffic_df['非TOP5月均销额'] = np.where(traffic_df['有销额竞品款数'] > 5,
                                               (traffic_df['利基月GMV'] - traffic_df['TOP5月销额']) / (
                                                       traffic_df['有销额竞品款数'] - 5),
                                               np.nan)

            traffic_df['非TOP5月均销额_score'] = common_util.get_cut(traffic_df, '非TOP5月均销额', para.l_sales_bins,
                                                                para.l_sales_labels)
            traffic_df['非TOP5月均销额_tag'] = common_util.get_cut(traffic_df, '非TOP5月均销额', para.l_sales_bins,
                                                              para.l_sales_tags)
            ...
            # E新品冒出
            # 平均开售月数
            traffic_revenue_month = calculation_util.product_mean(traffic_revenue_pass, 'related_asin', '开售月数',
                                                                  '平均开售月数')

            traffic_df = traffic_df.merge(traffic_revenue_month, how='left', on='related_asin')

            traffic_df['平均开售月数_score'] = common_util.get_cut(traffic_df, '平均开售月数', para.e_month_bins,
                                                             para.e_month_labels)
            ...
            # 冒出品新品产品数
            traffic_good_new_count = calculation_util.product_count(traffic_good_new, 'related_asin', 'asin',
                                                                    '冒出品新品产品数')
            ...
            traffic_df['平均星数_score'] = common_util.get_cut(traffic_df, '平均星数', para.e_ratings_bins,
                                                           para.e_ratings_labels)
            ...
            # 月销增长率不为空产品数
            traffic_sales_increase = traffic_revenue_pass.loc[
                traffic_revenue_pass['monthly_revenue_increase'].notnull()]

            traffic_sales_increase_count = calculation_util.product_count(traffic_sales_increase, 'related_asin',
                                                                          'asin',
                                                                          '月销增长率不为空产品数')
            ...
            # 平均留评率
            traffic_ratings_rate_mean = calculation_util.product_mean(traffic_revenue_pass, 'related_asin',
                                                                      'reviews_rate',
                                                                      '平均留评率')
            ...
            # 留评率标准差
            traffic_ratings_rate_std = calculation_util.product_std(traffic_revenue_pass, 'related_asin',
                                                                    'reviews_rate',
                                                                    '留评率标准差')
            ...
            traffic_ratings_rate = calculate_util.product_recommend(traffic_revenue_pass, 'reviews_rate',
                                                                    'monthly_revenue',
                                                                    '加权留评率')
            ...
            traffic_list_2 = ['综合竞品推荐度', '有销额竞品款数占比', '价格中位数', '预估平均资金利用率', '加权FBA运费', '动销品平均星级', 'TOP5销额占比',
                              '非TOP5销额占比', '代表度', 'P得分', 'M得分', 'PMI得分']

            for traffic_k in traffic_list_2:
                traffic_df[traffic_k] = data_cleaning_util.convert_type(traffic_df, traffic_k, 2)

            traffic_list_3 = ['达标推荐度占比', '价格集中度', '预估平均毛利率', 'FBM配送占比', '直发FBM产品占比', '直发FBM销额占比', 'AMZ直营销额占比',
                              '大牌商标销额占比', '中国卖家占比', '冒出品A+占比', '冒出品视频占比', '冒出品QA占比', '冒出品低星占比', '冒出品新品占比', '销级星数比',
                              '三标新品占比', '月销增长率', '加权留评率']
            ...
            df_group_tag = df_group[field.df_group_tag_pre_list]
            ...
            group_tag_list_05 = ['有销额竞品款数占比', '达标推荐度占比', '价格集中度', 'FBM配送占比', '直发FBM产品占比', '直发FBM销额占比', 'AMZ直营销额占比',
                                 '大牌商标销额占比', '中国卖家占比', '冒出品A+占比', '冒出品视频占比', '冒出品QA占比', '冒出品低星占比', 'TOP5销额占比',
                                 '非TOP5销额占比', '冒出品新品占比', '销级星数比', '三标新品占比', '月销增长率']
            ...
            df_group_tag['data_id'] = df_group_tag['related_asin'] + " | " + site + " | " + update_date
            ...
            df_traffic_recommend['site'] = site

            # 竞品表
            df_traffic_table = df_traffic_recommend[field.df_traffic_table_list]
            ...
            df_traffic_table.rename(columns=field.df_traffic_table_rename_dict, inplace=True)

            # 聚合表
            df_group = df_group[field.df_group_table_list]
            df_group = duplicate_util.df_cleaning(df_group, 'related_asin')

            df_group.rename(columns=field.df_group_rename_dict, inplace=True)

            # 聚合tag表
            df_group_tag = df_group_tag[field.df_group_tag_table_list]
            ...
            sql_engine.data_to_sql(
                df_traffic_table, path.product_traffic_history, 'append', config.connet_product_db_sql)
