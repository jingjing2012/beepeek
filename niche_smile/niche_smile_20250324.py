...
def get_previous_saturday(weeks_back: int = 2) -> str:
    """
    获取前N周的周六日期

    Args:
        weeks_back: 向前推几周，默认2周（上上周）
    ...
    # 计算到上上周六的天数
    days_to_saturday = current_weekday + 2  # 转换到周六需要加2
    days_back = days_to_saturday + (7 * weeks_back)  # 再往前推两周
...
for site in sites:
    ...
    while row_start < row_end:
        ...
        # 字段类型修正
        # niche表
        niche_list_1 = ["search_volume_T90", "search_volume_t_360", "minimum_units_sold_t_90",
                        "maximum_units_sold_t_90",
        ...
        df_keywords_CR['CR_KW'] = np.fmin(1, np.where(df_keywords_CR['总点击量'] * 1 > 0,
                                                      round(df_keywords_CR['总订单量_KW'] / df_keywords_CR['总点击量'], 4),
                                                      np.nan))
        ...
        df_niche['总售出件数_360'] = round((df_niche['minimum_units_sold_t_360'] + df_niche['maximum_units_sold_t_360']) / 2,
                                      0)

        # 平均每单件数_KW
        df_niche = df_niche.merge(df_keywords_CR, how="left", on="niche_id")
        df_niche['平均每单件数_KW'] = np.where(df_niche['总订单量_KW'] * 1 > 0,
                                         round(df_niche['总售出件数_360'] / df_niche['总订单量_KW'], 2),
                                         np.nan)
        ...
        # 完善表格
        df_cal_niche = df_niche.merge(df_scr_avg, how="left", on="niche_id").merge(df_scr_max, how="left",
                                                                                   on="niche_id") \
        ...
        if df_asin_w_avg.empty:
            ...
            niche_smile_m_list = ['是否探索', 'Monetary', '平均价格', '加权平均价', '体积超重占比', '重量', '体积重', '头程', '实抛偏差率', '规格分布',
                                  'FBA',
                                  '头程占比', 'FBA占比', '货值占比', 'FBA货值比', '资金利用效率1', '资金利用效率2', '资金利用效率AVG', '营销前毛利反推',
                                  '实抛分布',
                                  '重量分布']
            ...
            df_asin_w_avg['实抛偏差率_asin'] = np.where(df_asin_w_avg['体积重量_asin'] * df_asin_w_avg['weight'] * 1 > 0,
                                                   round(df_asin_w_avg['计算重量_asin'] / df_asin_w_avg['实抛偏差率_asin_a'] - 1,
                                                         4),
                                                   np.nan)
            ...
            df_asin_w_avg['头程占比_asin'] = np.where(df_asin_w_avg['头程_asin'] * 1 > 0,
                                                  round(df_asin_w_avg['头程_asin'] / df_asin_w_avg['avg_price'], 4),
                                                  np.nan)
            ...
            df_cut(df_weight_cal_w_avg, '计算重量', '重量分布', [-99, 0.01, 0.3, 1, 5, 10, 100],
                   ["", "轻小", "偏轻", "常规", "较重", "超重"])
            ...
            df_size_w_avg = df_price_w_avg.merge(df_referral_w_avg, how="left", on="niche_id") \
                ...
                .merge(df_weight_r_w_avg, how="left", on="niche_id").merge(df_weight_v_r_w_avg, how="left",
                                                                           on="niche_id") \
            ...
            conditions = [(df_size_w_avg['实抛偏差率'] >= 1) & (df_size_w_avg['体积超重占比'] >= 0.5),
                          (df_size_w_avg['实抛偏差率'] >= 0.2) & (df_size_w_avg['实抛偏差率'] < 1) & (
                                  df_size_w_avg['体积超重占比'] >= 0.5),
            ...
            df_size_w_avg['资金利用效率1'] = np.where(df_size_w_avg['货值占比'] * 1 > 0,
                                                round(df_size_w_avg['资金利用效率1_a'] / df_size_w_avg['资金利用效率1_b'], 2),
                                                np.nan)
            df_size_w_avg['资金利用效率2_a'] = np.where(df_size_w_avg['货值占比'] * 1 > 0,
                                                  1 - df_size_w_avg['佣金占比'] - para.exchange_loss - df_size_w_avg[
                                                      '货值占比'] -
                                                  df_size_w_avg['FBA占比'] -
            ...
            df_size_w_avg['资金利用效率2'] = np.where(df_size_w_avg['货值占比'] * 1 > 0,
                                                round(df_size_w_avg['资金利用效率2_a'] / df_size_w_avg['资金利用效率2_b'], 2),
                                                np.nan)
        ...
        if df_asin_launch_date.empty:
            ...
            # 计算上架天数
            df_asin_launch_date['上架天数'] = (
                    df_asin_launch_date['dataset_date'] - df_asin_launch_date['launch_date']).dt.days
        ...
        df_niche_size = df_cal_size[
            ['利基站点', 'mkid', 'niche_id', 'niche_title', '是否探索', '加权平均价_x', '体积重', '体积超重占比', 'FBA', '重量', '体积重', '头程',
             '实抛偏差率', 'ASIN平均上架年数', '总点击量_y', 'CR_KW', 'dataset_date', 'ASIN1', 'ASIN1品牌', 'ASIN1完整类名', 'ASIN2',
             'ASIN2品牌',
        ...
        # TOP5平均月销额
        df_niche_smile_pre['TOP5平均月销额'] = np.where(
            df_niche_smile_pre['top5_products_click_share_t360_count_yoy_x'] * 1 > 0,
        ...
        # 知名品牌
        conditions_brand = [
            (df_niche_smile_pre['top5_brands_click_share_t360_count_yoy_x'] * 1 > 0.9) & (
                    df_niche_smile_pre['Scale'] > 3),
            (df_niche_smile_pre['top5_brands_click_share_t360_count_yoy_x'] * 1 > 0.82) & (
                    df_niche_smile_pre['Scale'] > 2),
        ...
        # 整理到表
        df_niche_smile_pre = df_niche_smile_pre.merge(df_niche_url, how='left', left_on='niche_id_x',
                                                      right_on='niche_id')

        df_niche_a = df_niche_smile_pre[
            ['利基站点', 'niche_title_x', 'mkid_x', '是否探索', 'ASIN1', 'ASIN1品牌', 'ASIN1完整类名', 'ASIN2', 'ASIN2品牌',
             'ASIN2完整类名',
             'ASIN3', 'ASIN3品牌', 'ASIN3完整类名', 'ASIN平均上架年数', '知名品牌', 'top5_brands_click_share_t360_count_yoy_x',
             '知名品牌依赖',
        ...
        if any(sales_potential_score) or any(asin_launch_date):
            ...
            df_niche_smile_pre['Emerging_bonus'] = np.where(df_niche_smile_pre['avg_review_count_currentvalue_x'] < 800,
                                                            0.5, 0) + np.where(df_niche_smile_pre['近半年新品成功率'] >= 0.6,
                                                                               0.5,
                                                                               0) + np.where(
                df_niche_smile_pre['avg_review_rating_currentvalue_x'] < 4.1, 0.5, 0) - np.where(
                df_niche_smile_pre['avg_review_count_currentvalue_x'] >= 8000, 0.5, 0)
        ...
        df_niche_smile_pre['CR_周SCR'] = np.fmin(1, np.where(df_niche_smile_pre['搜索点击比'] * 1 > 0,
                                                            df_niche_smile_pre['周SCR_AVG'] / df_niche_smile_pre[
                                                                '搜索点击比'],
        ...
        df_niche_smile_pre['转化净值'] = np.where(df_niche_smile_pre['mkid_x'] == "UK", df_niche_smile_pre['计算价格_UK'] *
                                              df_niche_smile_pre['CR'] * np.fmin(2,
                                                                                 df_niche_smile_pre['平均每单件数_x'].pow(
                                                                                     0.9)),
        ...
        df_niche_smile_pre['计算广告占比档'] = (np.fmax(np.where(df_niche_smile_pre['搜索广告占比档'] * 1 > 0,
                                                          df_niche_smile_pre['搜索广告占比档'], para.bl2),
                                                 para.e) / para.bl2) ** (
        ...
        # bf(CC)和f(SR,AR)
        df_niche_smile_pre['bf(CC)和f(SR,AR)'] = para.d * (
                1 - df_niche_smile_pre['f(CC)'] / df_niche_smile_pre['f(SR,AR)'])
        ...
        df_niche_smile_pre['转化净值偏差'] = np.where(df_niche_smile_pre['mkid_x'] == "US", df_niche_smile_pre['转化净值偏差_US'],
                                                np.where(df_niche_smile_pre['mkid_x'] == "UK",
                                                         df_niche_smile_pre['转化净值偏差_UK'],
                                                         df_niche_smile_pre['转化净值偏差_OT']))
        ...
        df_niche_smile_pre['毛估CPC因子'] = np.fmax(
            para.p1 * df_niche_smile_pre['转化净值偏差'] * df_niche_smile_pre['f(SR,AR)'] + para.p2 * df_niche_smile_pre[
                'f(VR)'] + para.p3 * df_niche_smile_pre['f(NS,SP)'] + para.p4 * df_niche_smile_pre['bf(CC)和f(SR,AR)'],
            0)
        ...
        if df_cpc.empty:
            ...
            if oe_database.startswith("oe_us"):
        ...
        df_niche_smile_pre['CPC因子'] = np.where(df_niche_smile_pre['转化净值'] * df_niche_smile_pre['CPC'] * 1 > 0,
                                               para.product_acos * df_niche_smile_pre['转化净值'] / df_niche_smile_pre[
                                                   'CPC'],
                                               np.nan)
        ...
        # 转化净值分级
        df_cut(df_niche_smile_pre, '转化净值偏差', '转化净值分级', [-99, 0.7, 0.9, 1.5, 2, 3, 99],
               ["很差", "较差", "正常", "良好", "优秀", "极棒"])
        df_niche_smile_pre['转化净值分级'] = df_niche_smile_pre['转化净值分级'].astype('object').fillna("-")

        # Involution值计算
        conditions_i = [
            (df_niche_smile_pre['综合蓝海度'] >= 4.25) & (
                    df_niche_smile_pre['top5_brands_click_share_t360_count_yoy_x'] < 0.75),
        ...
        df_niche_i = df_niche_smile_pre[
            ['利基站点', 'niche_title_x', 'mkid_x', '是否探索', 'Involution', '平均价格', '总点击量', '平均每单件数_x', '转化净值', '转化净值偏差',
             ...
             'sponsored_products_percentage_t360_count_qoq_x', 'sponsored_products_percentage_t360_count_yoy_x',
             '平均缺货率',
